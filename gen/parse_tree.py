...
def translateClassName(name):
    name  = name.split(' ')[-1]
    parts = name.split('.')
    return parts[-2] if len(parts) > 1 and not parts[-2] == 'cv' else ''

def translateClass( defn):
    cname= translateName(defn[0])
    base = None
    if len(defn[1])>0 and(defn[1].find(':')==0) : 
        base = defn[1].replace(': ','').replace('cv::','')
        bazez= base.split(',')
        if len(bazez)>1:
            base = bazez[-1:1]
    c = Class(cname, '', [], [], base)
    #~ print( "tcb " + str(base) + " " + str(defn) )
    #~ print "tc  " + str(c)
    return c
...
class ParseTree(object):
    ...
    are manipulated toproduce capi sources.
    ...
    def build(self, namespaces):
        ...
        for name, definitions in namespaces.items():
            ...
            for defn in definitions:
                ...
                if obj is None:
                    print("no object for " + str(defn))
                    continue
                if type(obj) is Class or obj.clss:
                    self.insertIntoClassTree(obj, class_tree,defn)
...
class Translator(object):
    ...
    def translateClass(self, defn):
        return translateClass(defn)
        #~ cname= self.translateName(defn[0])
        #~ base = defn[1]
        #~ if len(base)>0 and(base.find(':')==0) : base = defn[1][1:]
        #~ cn = base.replace(': cv::','')
        #~ b = None
        #~ if base != cn:  b = [cn]
        #~ c = Class(cname, '', None, None, b)
        #~ print( "tcb " + base + " " + str(defn) )
        #~ print "tc  " + str(c)
        #~ return c
...
class Class(object):
    ...
    Class
      ...
      |- Methods
    """
    def __init__(self, name='', namespace='', constants=None, methods=None, bases=None):
        self.name = name
    ...
    def __str__(self):
        ...
        if self.bases is not None:      ret += " : " + join((c for c in self.bases),',').strip(',') 
        ret += ' {\n\t'
        if self.constants is not None : ret += join((c.__str__() for c in self.constants),'\n\t')+'\n\n\t'
        if self.methods is not None :   ret += join((f.__str__() for f in self.methods),'\n\t')
        ret += '\n};'
        return ret
